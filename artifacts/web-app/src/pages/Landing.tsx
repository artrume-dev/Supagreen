  ...
  return (
    <div className="min-h-screen bg-[#0F1710] font-sans text-white overflow-x-hidden">
      <nav className="sticky top-0 z-50 bg-[#0F1710]/90 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map((l) => (
              <a key={l} href={`#${l.toLowerCase().replace(/\s+/g, "-")}`} className="text-white/60 hover:text-white text-sm transition-colors">
                {l}
              </a>
            ))}
          </div>
          ...
          <div className="hidden md:flex items-center gap-3">
            {isLoggedIn ? (
              <Link href={appHref} className="bg-[#22C55E] hover:bg-green-400 text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition-all shadow-lg shadow-green-900/30">
                Open App
              </Link>
            ) : (
              <>
                <a href={authHref} className="text-white/60 hover:text-white text-sm transition-colors px-3 py-2">
                  Log in
                </a>
                <a href={authHref} className="bg-[#22C55E] hover:bg-green-400 text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition-all shadow-lg shadow-green-900/30">
                  Start free &rarr;
                </a>
              </>
            )}
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-white/5 bg-[#0F1710]/95 backdrop-blur-xl px-6 py-4 space-y-3">
            {navLinks.map((l) => (
              <a key={l} href={`#${l.toLowerCase().replace(/\s+/g, "-")}`} onClick={() => setMobileMenuOpen(false)} className="block text-white/60 hover:text-white text-sm py-1">
                {l}
              </a>
            ))}
            <div className="pt-2 border-t border-white/5 flex gap-3">
              {isLoggedIn ? (
                <Link href={appHref} className="flex-1 text-center bg-[#22C55E] text-white text-sm font-semibold px-5 py-2.5 rounded-xl">
                  Open App
                </Link>
              ) : (
                <>
                  <a href={authHref} className="text-white/60 text-sm py-2.5 px-3">Log in</a>
                  <a href={authHref} className="flex-1 text-center bg-[#22C55E] text-white text-sm font-semibold px-5 py-2.5 rounded-xl">
                    Start free
                  </a>
                </>
              )}
      ...
      <section className="relative overflow-hidden">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[600px] bg-[#22C55E]/[0.08] rounded-full blur-3xl pointer-events-none" />
        <div className="absolute top-20 right-0 w-[400px] h-[400px] bg-[#F97316]/[0.06] rounded-full blur-3xl pointer-events-none" />
          ...
          <div className="flex flex-col lg:flex-row items-center gap-16">
            ...
            >
              <div className="inline-flex items-center gap-2 bg-[#22C55E]/10 border border-[#22C55E]/20 text-[#22C55E] text-xs font-semibold px-4 py-2 rounded-full mb-6">
                <span>&#10024;</span>
                <span>Powered by AI &middot; 100% whole foods</span>
              </div>
              ...
              <p className="text-white/60 text-lg lg:text-xl leading-relaxed mb-8 max-w-xl mx-auto lg:mx-0">
                NutriSnap generates 3 personalised, whole-food recipes every morning &mdash; based on your diet, goals, and what&rsquo;s in season near you. No meal plans. No tracking hell. Just real food, made simple.
              </p>

              <div className="flex flex-col sm:flex-row gap-3 justify-center lg:justify-start mb-10">
                {isLoggedIn ? (
                  <Link href={appHref} className="bg-[#22C55E] hover:bg-green-400 text-white font-bold px-8 py-4 rounded-2xl text-base transition-all shadow-xl shadow-green-900/30 flex items-center justify-center gap-2">
                    <span>Go to Dashboard</span>
                    <span>&rarr;</span>
                  </Link>
                ) : (
                  <a href={authHref} className="bg-[#22C55E] hover:bg-green-400 text-white font-bold px-8 py-4 rounded-2xl text-base transition-all shadow-xl shadow-green-900/30 flex items-center justify-center gap-2">
                    <span>Start free &mdash; no card needed</span>
                    <span>&rarr;</span>
                  </a>
                )}
              </div>
            ...
            >
              <div className="relative w-80 h-[580px]">
                ...
                <div className="absolute left-1/2 -translate-x-1/2 top-0 w-64 h-[540px] bg-[#0F1710] rounded-[44px] border-4 border-white/20 shadow-2xl overflow-hidden z-10">
                  ...
                  <div className="p-4">
                    <div className="flex justify-between items-center mb-3">
                      <div>
                        <p className="text-white/50 text-[10px]">Today</p>
                        <p className="text-white font-bold text-sm">Today&rsquo;s Recipes &#10003;</p>
                      </div>
                      <div className="bg-[#1C2B1E] rounded-xl px-2 py-1.5 text-center">
                        <p className="text-xl">{"\u{1F525}"}</p>
                        <p className="text-[#F97316] text-[10px] font-bold">14d</p>
                      </div>
                    </div>

                    {[
                      { emoji: "\u{1F951}", title: "Avocado Power Bowl", time: "12 min", score: "9.4" },
                      { emoji: "\u{1F957}", title: "Mediterranean Salad", time: "18 min", score: "9.7" },
                    ].map((r, i) => (
                    ...
                    <div className="mt-3 bg-gradient-to-r from-[#22C55E] to-emerald-600 rounded-xl py-2.5 text-center">
                      <p className="text-white text-[11px] font-bold">{"\u{1F916}"} Ask NutriAI</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="absolute -right-6 top-32 bg-[#1C2B1E] border border-white/10 rounded-2xl px-3 py-2 shadow-xl z-20">
                  <p className="text-white text-xs font-bold">{"\u{1F3C6}"} 30-day streak!</p>
                ...
                <div className="absolute -left-6 bottom-28 bg-[#22C55E]/90 rounded-2xl px-3 py-2 shadow-xl z-20">
                  <p className="text-white text-xs font-bold">42g protein {"\u{1F4AA}"}</p>
      ...
      <div className="border-y border-white/5 py-6">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="flex items-center justify-center gap-10 flex-wrap">
            {["\u{1F33F} Whole Foods", "\u{1F33A} Trader Joe's", "\u{1F170} ALDI", "\u{1F3EA} Tesco", "\u{1F6D2} Sainsbury's", "\u{1F33F} Waitrose"].map((s) => (
        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <p className="text-white/50 text-lg max-w-xl mx-auto">
              From AI recipe generation to in-store navigation &mdash; NutriSnap is the only tool you need to eat well, consistently.
            </p>
          </div>
      ...
      <section id="how-it-works" className="py-24 bg-[#1C2B1E]/30">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-16">
      ...
      <section className="py-24">
        <div className="max-w-6xl mx-auto px-6">
              ...
              <div key={t.name} className="bg-[#1C2B1E] rounded-3xl p-6 border border-white/5">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-white/40 text-xs">{t.handle} &middot; {t.goal}</p>
                  </div>
                </div>
          ...
          <div className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-5">
            {[
              { val: "50,000+", label: "Active users worldwide", icon: "\u{1F465}" },
              { val: "2.1M+", label: "Whole-food recipes generated", icon: "\u{1F37D}" },
      ...
      <section id="pricing" className="py-24 bg-[#1C2B1E]/30">
        <div className="max-w-5xl mx-auto px-6">
          <div className="text-center mb-8">
            ...
            <p className="text-white/50 text-lg max-w-lg mx-auto">
              Start free forever. Upgrade when you&rsquo;re ready for the full experience.
            </p>
          </div>
      ...
      <section className="py-24 relative overflow-hidden">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[400px] bg-[#22C55E]/[0.08] rounded-full blur-3xl pointer-events-none" />
        <div className="relative max-w-3xl mx-auto px-6 text-center">
          <div className="text-7xl mb-6">{"\u{1F957}"}</div>
          ...
          <p className="text-white/50 text-xl mb-8 max-w-lg mx-auto">
            Join 50,000+ people who&rsquo;ve made whole-food eating effortless with NutriSnap.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            {isLoggedIn ? (
              <Link href={appHref} className="bg-[#22C55E] hover:bg-green-400 text-white font-bold px-10 py-4 rounded-2xl text-base transition-all shadow-2xl shadow-green-900/30">
                Go to Dashboard &rarr;
              </Link>
            ) : (
              <a href={authHref} className="bg-[#22C55E] hover:bg-green-400 text-white font-bold px-10 py-4 rounded-2xl text-base transition-all shadow-2xl shadow-green-900/30">
                Get started free &rarr;
              </a>
            )}
          </div>
          <p className="text-white/30 text-xs mt-5">No credit card required &middot; Cancel anytime &middot; GDPR compliant</p>
        </div>
      </section>
      ...
      <footer className="border-t border-white/5 py-12">
        <div className="max-w-6xl mx-auto px-6">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-8 mb-10">
            <div className="col-span-2">
              ...
              <div className="flex gap-3 mt-4">
                {["\u{1D54F}", "\u{1F4F8}", "\u{1F3B5}"].map((s) => (
                  <span key={s} className="w-8 h-8 bg-white/5 hover:bg-white/10 rounded-lg flex items-center justify-center text-sm transition-all cursor-default">
                    {s}
                  </span>
                ))}
              </div>
            ...
            {[
              ...
              <div key={col.title}>
                ...
                <ul className="space-y-2">
                  {col.links.map((l) => (
                    <li key={l}>
                      <span className="text-white/40 hover:text-white text-sm transition-colors cursor-default">{l}</span>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div className="border-t border-white/5 pt-6 flex flex-col md:flex-row items-center justify-between gap-3">
            <p className="text-white/30 text-xs">&copy; {new Date().getFullYear()} NutriSnap. All rights reserved.</p>
            <p className="text-white/30 text-xs">Powered by Claude AI &middot; 100% whole foods, always.</p>
          </div>
        </div>
      </footer>
    </div>
