...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div
      className="bg-background font-sans max-w-[420px] mx-auto flex flex-col shadow-2xl shadow-black"
      style={{ height: "100dvh" }}
    >
      ...
      {showNav && (
        <nav className="flex-shrink-0 glass-panel border-t border-white/5 px-6 py-3 flex justify-between items-center pb-safe">
          {NAV_ITEMS.map((item) => {
            const isActive = location === item.id;
            return (
              <Link key={item.id} href={item.id} className="relative group flex-1 flex justify-center">
